...
*/
    ...
    class Cart {
        constructor() {
            ...
            this.cartCounter = document.querySelector("[data-cart-counter]");
        ...
        initCartControls() {
            ...
            this.cartItemsContainer.addEventListener("click", (e => {
                ...
                if (target.closest("[data-quantity-decrease]")) {
                    const productId = target.closest("[data-cart-item]")?.dataset.cartItem;
                    if (productId) this.decreaseQuantity(productId);
                } else if (target.closest("[data-quantity-increase]")) {
                    const productId = target.closest("[data-cart-item]")?.dataset.cartItem;
                    if (productId) this.increaseQuantity(productId);
            ...
            document.addEventListener("click", (e => {
                const checkoutBtn = e.target.closest(".cart__checkout");
                if (checkoutBtn) {
                    e.preventDefault();
        ...
        showCheckoutForm() {
            ...
            closeBtn.addEventListener("click", (() => this.closeCheckoutForm(modal)));
            modal.addEventListener("click", (e => {
                if (e.target === modal) this.closeCheckoutForm(modal);
            }));
            const form = modal.querySelector("#checkoutForm");
            form.addEventListener("submit", (e => this.handleCheckoutSubmit(e, modal)));
        }
        closeCheckoutForm(modal) {
            modal.classList.remove("is-active");
            setTimeout((() => modal.remove()), 300);
        }
        handleCheckoutSubmit(e, modal) {
            e.preventDefault();
            const form = e.target;
            const formData = new FormData(form);
            const data = Object.fromEntries(formData.entries());
            let isValid = true;
            const inputs = form.querySelectorAll(".checkout-modal__input");
        ...
        getProductData(productCard) {
            ...
            const productImage = productCard.querySelector(".product-card__image-ibg img")?.src;
            ...
            const productPrice = priceElement?.dataset.productPrice;
            if (!productId || !productImage || !productTitle || !productPrice) {
        ...
        addToCart(productData) {
            ...
            if (existingItem) existingItem.quantity++; else this.items.push(productData);
            this.saveCart();
            this.updateCartUI();
        }
        ...
        updateCartUI() {
            if (!this.cartItemsContainer || !this.cartSummary) return;
            const cartEmpty = document.querySelector(".cart__empty");
            const cartSummaryElement = document.querySelector(".cart__summary");
            ...
            const shipping = subtotal > 5e4 ? 0 : 5990;
            const total = subtotal + shipping;
            if (this.cartSummary) this.cartSummary.innerHTML = this.createCartSummaryHTML(subtotal, shipping, total);
        }
        createCartItemHTML(item) {
            if (!item || !item.id || !item.image || !item.title || !item.price) {
                console.error("Invalid cart item:", item);
                return "";
            }
            return `\n            <div class="cart__item" data-cart-item="${item.id}">\n                <div class="cart__item-image">\n                    <img src="${item.image}" alt="${item.title}">\n                </div>\n                <div class="cart__item-content">\n                    <h3 class="cart__item-title">${item.title}</h3>\n                    <div class="cart__item-price">$${(item.price * item.quantity).toLocaleString("es-CL")}</div>\n                    <div class="cart__item-quantity">\n                        <button class="cart__item-btn" data-quantity-decrease>\n                            <i class="fas fa-minus"></i>\n                        </button>\n                        <span data-quantity>${item.quantity}</span>\n                        <button class="cart__item-btn" data-quantity-increase>\n                            <i class="fas fa-plus"></i>\n                        </button>\n                    </div>\n                </div>\n                <button class="cart__item-remove" data-remove-item>\n                    <i class="fas fa-trash"></i>\n                </button>\n            </div>\n        `;
        }
        createCartSummaryHTML(subtotal, shipping, total) {
            return `\n            <div class="cart__summary-row">\n                <span>Subtotal:</span>\n                <span>$${subtotal.toLocaleString("es-CL")}</span>\n            </div>\n            <div class="cart__summary-row">\n                <span>Envío:</span>\n                <span>${shipping === 0 ? "Gratis" : "$" + shipping.toLocaleString("es-CL")}</span>\n            </div>\n            <div class="cart__summary-row cart__summary-total">\n                <span>Total:</span>\n                <span>$${total.toLocaleString("es-CL")}</span>\n            </div>\n            ${total > 0 ? `\n                <button class="cart__checkout button button--primary">\n                    <i class="fas fa-shopping-cart"></i>\n                    <span>Finalizar Compra</span>\n                </button>\n            ` : ""}\n        `;
        }
        calculateSubtotal() {
    ...
    const products = [ {
        id: 1,
        title: "Catan",
        description: "El juego de mesa más vendido del mundo",
        ...
        id: 2,
        title: "Monopoly",
        description: "El clásico juego de negocios inmobiliarios",
        image: "img/products/monopoly.jpg",
        rating: 4.8,
        ...
        id: 3,
        title: "UNO",
        description: "El juego de cartas más divertido",
        image: "img/products/uno.jpeg",
        rating: 4.7,
        ...
        id: 4,
        title: "Jenga",
        description: "El clásico juego de habilidad y equilibrio",
        image: "img/products/jenga.jpg",
        rating: 4.6,
        ...
        description: "NOALED Futbolín Profesional",
        image: "img/products/ft.jpg",
        rating: 4.9,
        ...
        id: 6,
        title: "Scrabble",
        description: "El mejor juego de palabras",
        image: "img/products/scrabble.jpg",
        rating: 4.7,
        players: "2-4 jugadores",
        time: "45 min",
        price: 22990,
        ...
        id: 7,
        title: "Risk",
        description: "El juego de estrategia militar más famoso",
        image: "img/products/risk.jpg",
        rating: 4.8,
        ...
        image: "img/products/pictionary.jpg",
        rating: 4.7,
    ...
    function initializeAddToCart() {
        ...
        addToCartButtons.forEach((button => {
            button.addEventListener("click", (function() {
                ...
                if (product) console.log("Added to cart:", product);
            }));
        }));
    }
    function initializeHelpfulButtons() {
        ...
        helpfulButtons.forEach((button => {
            ...
            if (!localStorage.getItem(`${reviewId}_likes`)) {
                const initialLikes = Math.floor(Math.random() * (1418 - 52 + 1)) + 52;
                localStorage.setItem(`${reviewId}_likes`, initialLikes);
                localStorage.setItem(`${reviewId}_liked`, "false");
            }
            const likesCount = localStorage.getItem(`${reviewId}_likes`);
            const countElement = button.querySelector(".review-card__helpful-count");
            countElement.textContent = likesCount;
            if (localStorage.getItem(`${reviewId}_liked`) === "true") button.classList.add("review-card__helpful--active");
        }));
        helpfulButtons.forEach((button => {
            button.addEventListener("click", (function() {
                const reviewId = this.dataset.reviewId;
                const isLiked = localStorage.getItem(`${reviewId}_liked`) === "true";
                const countElement = this.querySelector(".review-card__helpful-count");
                let currentLikes = parseInt(localStorage.getItem(`${reviewId}_likes`));
                if (!isLiked) {
                    currentLikes++;
                    this.classList.add("review-card__helpful--active");
                    localStorage.setItem(`${reviewId}_liked`, "true");
                }
                localStorage.setItem(`${reviewId}_likes`, currentLikes);
        ...
        setInterval((() => {
            helpfulButtons.forEach((button => {
                ...
                const shouldIncrement = Math.random() < .3;
                if (shouldIncrement) {
                    const countElement = button.querySelector(".review-card__helpful-count");
                    let currentLikes = parseInt(localStorage.getItem(`${reviewId}_likes`));
                    currentLikes++;
